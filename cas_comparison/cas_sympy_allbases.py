"""
SymPy CAS comparison for any basis.
MPI-parallel. Same methodology as cas_full_sympy.py but takes --pkl and --out args.

Usage:
  mpirun -n 10 python3 cas_sympy_allbases.py --pkl results_core_maths_k10.pkl --out cas_sympy_core_maths.json
"""
import json
import pickle
import time
...
def main():
    # Parse args on all ranks (MPI-safe)
    ...
    if rank == 0:
        ...
        for s, c in strats.most_common():
            print(f"    {s}: {c}")
        print(f"Failed: {n_failed}, Timeout: {n_timeout}, Error: {n_error}")
