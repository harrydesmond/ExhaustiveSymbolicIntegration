...
\newcommand{\codex}[1]{{\color{purple}\textbf{[Codex: #1]}}}
\newcommand{\codexadd}[1]{{\color{purple}#1}}

\definecolor{teal}{rgb}{0,0.5,0.5}
\newcommand{\claude}[1]{{\color{teal}\textbf{[Claude: #1]}}}
...
%\hd{five?}
We also deploy ESI as a novel integration algorithm, identifying 5 antiderivatives
%\codexadd{within the four bases included in the full CAS cascade}
that resist SymPy, Mathematica, RUBI, FriCAS, Maxima and Giac.
When an antiderivative can be found by both ESI and computer algebra systems, the latter often returns a simpler form.
%Among integrals that both ESI and standard computer algebra systems (CAS) successfully evaluate, \codexadd{ESI is not more often simpler than SymPy, but its wins are substantially larger in total node-count savings.}
...
These results reveal how that the landscape of symbolic integrability is shaped primarily by the choice of operators, and that exhaustive enumeration can systematically discover integrable forms --- including novel ones --- that elude general-purpose algorithms.
\end{abstract}
...
ESI also affords an integration algorithm in its own right, locating antiderivatives by searching an exhaustive catalogues for functions whose derivatives match the targets. This can be used both to find anderivatives that fall through the cracks of computer algebra systems' Risch implementations, and provide a way to rapidly produce simple antiderivative forms.
...
%\codexadd{Across the four bases included in the CAS cascade, ESI identifies 5 integrals that resist all six CAS engines under the standard call.} \codexadd{A broader intermediate set in \texttt{ext\_log\_maths} contains 232 integrals unsolved by SymPy, Mathematica and RUBI, but FriCAS solves 218 of these, showing that most such failures reflect implementation coverage rather than intrinsic CAS resistance.}
%We also solve two integrals from the RUBI test suite that RUBI itself cannot evaluate.
%\codexadd{When ESI and SymPy both solve an integral, ESI is not more often simpler, but its wins are substantially larger in total node-count savings.}
...
and nor has exhaustive enumeration been used to systematically discover novel closed-form integrals.
...
and directions for future work. Sec.~\ref{sec:conclusion} concludes.
...
As noted in \cite{Bartlett2024} and subsequent applications this is an imperfect procedure, with many duplicates remaining due to the SymPy's imperfect simplification procedure. We therefore apply a second deduplication layer via numerical fingerprinting (Section~\ref{sec:fingerprint}) to catch additional duplicates.
...
These counts define the denominator of~$\rhok$.  Table~\ref{tab:sizes} gives the cumulative function-space sizes and the CAS comparison counts used later. \codexadd{The \texttt{core\_log\_maths} basis was introduced to investigate the effect of log in the integrability-fraction analysis and is not included in the full six-engine CAS cascade, although we do report the available multi-member count and any lightweight CAS checks completed for it.}

Generating the function space is the most expensive step of the entire procedure: the cost grows rapidly with $k$ because the number of candidate trees is exponential (this is why we extend \texttt{core\_maths}, containing relatively few operators, to $k=10$ but the others only to $k=9$). \codexadd{For the larger bases at their top complexity this is expensive: tree generation for \texttt{ext\_log\_maths} at $k=9$ requires ${\sim}20$ CPU-hours, and symbolic deduplication of the resulting 56 million candidate trees requires several days on tens of cores with ${\sim}20$--30\,GB of memory per core.}
%total
...
\midrule
\texttt{core\_maths} (10)     & 77{,}053  & 13{,}993  & 1{,}059       & \codexadd{218}  & 2{,}478       & 411            \\
\texttt{core\_log\_maths} (8) & 21{,}214   & \codexadd{5{,}669}       & \codexadd{144}           & \codexadd{59}  & ---           & ---            \\
...
\texttt{ext\_log\_maths} (8)  & 222{,}838  & 49{,}538  & 2{,}431       & \codexadd{${\geq}232$}   & \codexadd{2{,}049}       & \codexadd{537}  \\
\texttt{trig\_maths} (9)      & 897{,}293 & 51{,}027 & 9{,}795    & 3{,}700 & \codexadd{685}       & \codexadd{435}            \\
\bottomrule
\end{tabular}
\end{table}
\codexadd{For computational reasons we do not fully complete this table; instead we fill entries that are needed to support the conclusions or that were cheaply available from the existing pipeline.  The ``tested'' column is the number of multi-member equivalence classes, the only classes on which ESI supplies a non-trivial lookup primitive.  ``Hard'' denotes failure in the initial sweep,
...
both fast and, as we demonstrate, extremely reliable.
...
For \texttt{ext\_log\_maths}, this numerical fingerprinting reduces the $k \leq 8$ $x$-dependent catalogue from 319{,}128 functions to 222{,}838 unique hashes, a reduction of $\sim$30\% (for \texttt{trig\_maths}, the analogous reduction is $\sim$4\%). The rate is particularly high for the \texttt{ext\_log\_maths} due to the many syntactically distinct but functionally equivalent ways of composing \texttt{log} and \texttt{Abs}.
Note that 
...
expressions related by parameter reparametrisation (e.g.\ $a_0 x$ vs.\ $a_0 a_1 x$, which are the same parametric family but evaluate differently at fixed parameter values) are not caught by this procedure.
...
Since each function independently either does or does not have its derivative in $\Fleq$, the count of integrable functions follows a binomial distribution with success probability $\rho$ and $n = |\Fleq|$ trials. We therefore report binomial uncertainties $\sqrt{\rho(1-\rho)/n}$ (noting that they do not take account of systematic uncertainties from the imperfect deduplication procedure). Differentiating and hashing $\sim 10^5$ functions takes $\sim 10$ minutes on a single core, while the hash-lookup step (computing $\rhok$ and building equivalence classes) is effectively instantaneous.
...
\item \emph{Rule-based:} RUBI \citep{Rubi2024} (Rule-Based Integration) relies exclusively on a curated decision tree of approximately 6{,}700 pattern-matching rules, implemented as a Mathematica package.
...
For integrals that resist all four, we additionally test Maxima \citep{Maxima2024} and Giac \citep{Giac2024} via SageMath. An integral that resists all six has therefore been tested against partial and near-complete Risch implementations, commercial heuristics, an independent rule-based system, and two further general-purpose CAS, strongly suggesting that it cannot be solved by any current CAS implementation.

We begin by stripping the \texttt{Abs}/\texttt{sign} wrappers that ESR inserts around arguments of \texttt{log}, \texttt{sqrt}, and \texttt{pow} as a precaution during tree enumeration. For SymPy we then apply simplification, try integration with \texttt{conds=`none'} and positive parameter assumptions, and fall back to SymPy's \texttt{manual=True} step-by-step mode if necessary. For Mathematica we pass \texttt{Assumptions -> \{x > 0, params > 0\}} in the initial sweep to match the positive evaluation domain used for fingerprinting and to choose a single real branch for logarithms and radicals, re-testing the main failures without this assumption and under alternative domains. For RUBI we call \texttt{Int[expr, x]}. For FriCAS, we convert the integrand to FriCAS's InputForm syntax (converting \texttt{atan2} to \texttt{atan} as it is called in that code) and call \texttt{integrate(expr, x)}. As a cross-check, we also differentiate the known ESI primitive within FriCAS and attempt to integrate the result to avoid any artifacts of the SymPy-to-FriCAS syntax conversion.
...
The central quantity in this study is the \emph{integrability fraction} (Eq.~\ref{eq:1}), which in the notation of Sec.~\ref{sec:method} can be written as
\begin{equation}
\label{eq:rho}
...
% \hd{This fig needs to be described fully, probably at the start of this para}
\codexadd{We characterise this using the \emph{complexity gap} $\delta \equiv \min\text{-complexity}(F') - \text{complexity}(F)$. For \texttt{ext\_log\_maths} at $k\leq8$ the mean $\delta = -1.13$ and $89.5\%$ of functions satisfy $\delta\leq0$: differentiation typically \emph{reduces} expression complexity. Log-containing functions shift this only marginally (mean $\delta=-1.20$, $89.9\%$ with $\delta\leq0$ vs $-1.03$ and $88.6\%$ for functions without log), so the ${\sim}2.5\times$ log enhancement cannot be attributed to complexity reduction alone.}
...
This high success rate confirms that the barrier for the heuristic and rule-based engines is not mathematical difficulty but incomplete coverage of exponential--logarithmic towers. FriCAS solves all $x^x$-type and $f(x)^{g(x)}$ integrals that defeat the other three engines. By contrast, RUBI solves none of the 232 integrals that SymPy and Mathematica leave unsolved because it is a pattern library designed for textbook integral forms and by design does not cover these deeply nested exp-log towers.
...
\codexadd{Extending this cascade to the remaining bases: for \texttt{ext\_maths}, a corrected no-\texttt{Assumptions} Mathematica rerun reduces the Abs candidate set from 163 to 2 failures; for \texttt{trig\_maths}, 685 Mathematica failures are cleared by Sage (502) and FriCAS (183), leaving zero survivors; \texttt{core\_maths} produces no all-engine failures.  A further independent broad search of 133{,}576 Abs-free $k=9$ candidates in \texttt{ext\_log\_maths} via a Mathematica--Sage--FriCAS cascade also leaves zero new survivors, validating the uniqueness of the Abs-free headline.  In total, Table~\ref{tab:cas_blind} contains 5 standard-call six-engine failures across the four CAS-tested bases: 3 that resist all 12 Mathematica variants tested, and 2 Abs-containing cases solved only under domain restriction or roundtrip.}
...
\begin{table*}
  \centering
  \caption{\codexadd{The 5 integrals that resist all six CAS engines under the standard integration call but are solved by ESI. ``Robust'' means that the integral also resists all alternative Mathematica strategies we tested; non-robust entries fail the standard call but are solved after a domain restriction or roundtrip transformation.}}
  %\hd{Explain here what Robust means. Also what do you mean ``under the standard integration call''? I thought they all failed under exhaustive testing.}
  %\hd{Again, shorten and simplify this caption, putting the detail in the main text.}
  %\hd{Put the robust column at the end}
  \label{tab:cas_blind}
  \small
  \begin{tabular}{clllc}
    \toprule
...
On github we provide a Python script and Jupyter notebook for solving integrals by ESI through a look-up to the exhaustive function lists.
%\codexadd{The lookup implementation used for this demonstration is \texttt{esi\_integrate.py}, with worked examples in \texttt{esi\_integrate\_demo.ipynb}.}
...
Liouville's theorem \citep{Liouville1835,Rosenlicht1972}, the foundational result of differential algebra, establishes that the elementary functions form a differential field closed under integration. It does not, however, say anything about the complexity of these integrals. The integrability fraction $\rhok$ that we introduce fills in this information, measuring the proportion of functions at complexity $k$ whose antiderivatives are also expressible at complexity $\leq k$ within the same operator basis. The declining $\rhok$ observed in the bases with high-complexity coverage (Section~\ref{sec:rho}) shows that the
%although every elementary function has an elementary antiderivative, that
antiderivatives increasingly requires higher complexity to express and hence increasingly fall outside the enumerated space. The $\sim 2.5\times$ elevation of $\rhok$ in \texttt{ext\_log\_maths} relative to \texttt{ext\_maths} for $k\geq 5$ reflects the approximate closure of the exp--log differential subsystem: the logarithm's derivative rule $\dd/\dd x[\log f] = f'/f$ produces rational forms that frequently match existing functions within the enumerated space, whereas the exponential's derivative $f'\exp(f)$ tends to increase complexity beyond the enumeration boundary. A quantitative model formalising this asymmetry is presented in~\ref{app:model}.
...
%\hd{shorten the codexadd text in the next few paras, focusing on higher-level discussion and eliminating repetition with the previous section}
The CAS comparison of Sec.~\ref{sec:cas} can therefore be seen as diagnosing incompleteness in current Risch implementations.
...
We therefore believe that we are the first to present an algorithm that finds the antiderivaties in Table~\ref{tab:cas_blind} automatically.
However, it is important to bear in mind that this is not because those integrals are fundamentally hard: one can guess the solutions with a little trial and error, as can modern large language models.

\subsection{Limitations and further work}

The main limitation of our $\rho(k)$ results is that they depend on ESR's specific expression grammar, which determines how the function space is partitioned by complexity. Changing the grammar or the complexity metric changes both the denominator of $\rhok$ and the location of features such as the \texttt{ext\_log\_maths} peak. Indeed, the non-monotonic peak at $k=6$ in the ESR node-count metric does not appear under alternative complexity functionals such as expression-tree depth, operator count, or reassignment of each function to the minimum complexity at which its numerical fingerprint first appears. \codexadd{For example, if the node-count metric is modified so that each \texttt{log} or \texttt{exp} operator contributes costs 2 rather than 1 unit of complexity, $\rhok$ instead flattens to $\sim 8$--$9\%$ with no discernible peak.}
%\hd{what does this mean?}

Another choice upon which the quantitative results depend in the deduplication method: under string-only deduplication, the peak disappears because syntactic identity misses the additional equivalences found by numerical fingerprinting. Future grammar-level improvements, including hash-based simplification methods \citep{hash,Kronberger2024}, may therefore shift the quantitative values and should be treated as a change in the measured function space.
...
The complexity ceiling of $k \leq 8$--$10$ restricts the analysis to relatively simple expressions; many primitives of practical interest lie at higher complexity. Quantitative results are specific to the chosen operator basis and complexity metric (node count).
...
five integrals \codexadd{in the four bases included in the full CAS cascade} that resist all tested CAS (SymPy, Mathematica, RUBI, FriCAS, Maxima, and Giac) but are readily identifiable by ESI. Three of these are \emph{robustly} CAS-resistant, failing every one of 12 Mathematica strategy combinations we tested; these three form a structurally coherent family pairing an algebraic extension $\sqrt{\cdot}$ with a transcendental tower ($e^x$, $e^{-x}$, or $x^x$) whose coupling cannot be inverted by a closed-form substitution. The remaining two are resistant due to the non-analyticity of the absolute value. We also solve two RUBI-failed benchmark integrals.
%\hd{was this discussed above and where are these integrals?}

ESI's value lies not in competing with CAS on integration in general, but in its exhaustive coverage: it catalogues every integrable function within the grammar, enabling both $\rhok$ measurement and the systematic discovery of integrals beyond the capabilities of existing algorithms. The method can readily be extended to higher complexity and alternative basis sets---this would shore up and generalise our integrability findings, unearth further as-yet unknown antiderivatives and extend our systematic exploration of limitations in existing Risch implementations.
